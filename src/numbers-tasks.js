...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorLength1 = Math.hypot(x1, y1);
  const vectorLength2 = Math.hypot(x2, y2);
  const cousine = scalar / (vectorLength1 * vectorLength2);
  return Math.acos(cousine);
}
...
function roundToPowerOfTen(num, pow) {
  const countPow = 10 ** pow;
  const countRound = Math.round(num / countPow);
  return countRound * countPow;
}
...
function getSumOfNumbers(...rest) {
  return rest.reduce((acc, count) => acc + count).toFixed(2);
}
